...
function ServerRack() {
  ...
  const [playerCount, setPlayerCount] = useState(1248)

  useEffect(() => {
    const interval = window.setInterval(() => {
      setPlayerCount((prev) => {
        const delta = Math.floor(Math.random() * 9) - 4
        return Math.min(1320, Math.max(1180, prev + delta))
      })
    }, 1000)
  ...
  return <motion.div initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .7, delay: .2 }} className="relative mx-auto mt-12 h-[385px] w-full max-w-[460px] lg:mt-0">
    ...
    <div className="absolute bottom-8 right-0 border border-lime-300/25 bg-[#0a100b]/95 px-4 py-3"><div className="flex items-center gap-2"><Server size={15} className="text-lime-300" /><div><p className="text-[11px] font-bold text-white">{playerCount.toLocaleString()}</p><p className="mt-0.5 text-[9px] tracking-wide text-zinc-500">PLAYERS ONLINE</p></div></div></div>
