...
export default function AuthPage({ mode }: { mode: 'login' | 'signup' }) {
  ...
  function handleProviderAuth(provider: AuthProvider) {
    ...
    window.setTimeout(() => { setAuthPending(null); navigate('/') }, 500)
  }
...
<div className="flex items-start justify-between gap-3 text-xs">{isSignup ? <label className="flex cursor-pointer items-start gap-2 text-zinc-500"><input type="checkbox" checked={values.terms} onChange={e => update('terms', e.target.checked)} className="mt-0.5 h-3.5 w-3.5 accent-violet-500" />I agree to the <Link to="/terms" className="text-violet-300 hover:text-violet-200">Terms of Service</Link></label> : <label className="flex cursor-pointer items-center gap-2 text-zinc-500"><input type="checkbox" checked={values.remember} onChange={e => update('remember', e.target.checked)} className="h-3.5 w-3.5 accent-violet-500" />Remember me</label>}{!isSignup && <Link to="/support" className="font-medium text-violet-300 hover:text-violet-200">Forgot password?</Link>}</div>
